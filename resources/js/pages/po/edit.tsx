...
export default function PO({ supplier = [], po }: Props) {
  const formatDate = (dateString: string | null): string => {
    if (!dateString) return ''
    // Handle "2026-02-23T00:00:00.000000Z" (ISO)
    // Handle "2026-02-23 00:00:00" (MySQL datetime)  
    // Handle "2026-02-23" (sudah bersih)
    return dateString.split('T')[0].split(' ')[0]
  }
  const { data, setData, put, processing, errors } = useForm<POForm>({
    VendAccount: po.VendAccount?.toString() ?? "",
    supplier_name: "",
    PurchId: po.PurchId ?? "",
    total_qty: 0,
    incoterm: po.incoterm ?? "",
    createdDateTime: formatDate(po.createdDateTime),           // ✅ pakai formatDate
    expected_delivery_date: formatDate(po.expected_delivery_date),  // ✅
    expected_arrival_date: formatDate(po.expected_arrival_date),  
    notes: po.notes ?? "",
    ...
    items: po.items?.length
      ? po.items.map((item) => ({
          ItemId: item.ItemId ?? "",
          Name: item.Name ?? "",
          PurchQty: item.PurchQty?.toString() ?? "",
          PurchUnit: item.PurchUnit ?? "pcs",
    ...
    <AppLayout>
      ...
      <div className="p-6 space-y-6 bg-linear-to-br from-zinc-50 to-zinc-100 min-h-screen">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-zinc-900">Edit Purchase Orders</h1>
            <p className="text-zinc-600 mt-1">Form Edit purchase orders</p>
          </div>
          <Link href="/po">
